window.addEventListener('load', () => {
  if (!document.querySelector('.one-application-creating')) {
    return null
  } else {
    const checkboxList = document.querySelectorAll(".one-application-creating_form_item_loading-types__checkboxes__item");

    checkboxList.forEach(function(checkboxList) {
      const oneAppCheckbox = checkboxList.querySelector('.one-application-creating_form_item_loading-types__checkboxes__item__input');
      const oneAppSvg = checkboxList.querySelector('.one-application-creating_form_item_loading-types__checkboxes__item__img');

      checkboxList.addEventListener('click', function() {
        oneAppCheckbox.checked = !oneAppCheckbox.checked;

        if (oneAppCheckbox.checked) {
          oneAppSvg.innerHTML = `<svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          checkboxList.classList.add('select-checkbox');
          checkboxList.querySelector('label').classList.add('select-checkbox-label');
        } else {
          oneAppSvg.innerHTML = `<svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          checkboxList.classList.remove('select-checkbox');
          checkboxList.querySelector('label').classList.remove('select-checkbox-label');
        }
