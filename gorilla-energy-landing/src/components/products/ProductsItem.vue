  ...
  .products__choosen(:class="`products__choosen_${item.code}`")
    ...
    .products__choosen-info.animated(ref="info")
      .products__choosen-bottle.product__bottle(
        v-if="$te(`products.bottle.${item.code}.text`)"
        v-lazyload
      ...
      .products__choosen-info-content(v-lazyload :class="`products__choosen-info-content_${item.code}`")
        ...
        .products__choosen-info-nutritional(v-if="item.nutritional")
          .products__choosen-info-nutritional-content(
            v-html="item.nutritional"
            :style="{ maxHeight: `${nutritionalHeight}px` }"
          )
          .products__choosen-info-nutritional-btn(
            :class="{ 'products__choosen-info-nutritional-btn_active': isActive }"
            @click="toggleNutritional"
          )
            span {{ btnTxt }}
        .products__choosen-info-list(v-html="item.composition" v-lazyload)
</template>
...
export default {
  ...
  computed: {
    ...
    nutritionalHeight () {
      return this.isActive ? this.$refs.info.querySelector('.products__choosen-info-nutritional-content').scrollHeight : 0
    }
  },
  watch: {
    item () {
