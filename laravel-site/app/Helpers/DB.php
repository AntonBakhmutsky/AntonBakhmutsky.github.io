...
class DB
{
  public static function createEnumColumn(string $tableName, string $columnName, array $values, ?string $defaultValue = null, ?bool $index = false): void
  {
    $typeName = Str::singular($tableName) . '_' . Str::plural($columnName);
    \DB::statement("drop type if exists $typeName cascade");
    \DB::statement("create type $typeName as enum('" . implode("', '", $values) . "')");
    \DB::statement("alter table $tableName add column $columnName $typeName" . ($defaultValue ? " not null default '$defaultValue'" : ""));
    if ($index === true) {
      \DB::statement("create index on $tableName ($columnName)");
    }
  }

  public static function dropEnumColumn(string $tableName, string $columnName)
  {
    $typeName = Str::singular($tableName) . '_' . Str::plural($columnName);
