		...
		<div id="DPD_info">
			...
			<div id="DPD_delivInfo_PVZ">
				<div>
					...
					<span id="DPD_pPrice"><?= GetMessage('IPOLH_DPD_PICKUP_COST_TEXT', ['#PRICE#' => $arResult['TARIFFS']['PICKUP']['COST']]) ?></span>,
                    
                    <?= GetMessage('IPOLH_DPD_PICKUP_PERIOD_SM') ?>
					<span id="DPD_pDate"><?= GetMessage('IPOLH_DPD_PICKUP_PERIOD_TEXT', ['#DAYS#' => $arResult['TARIFFS']['PICKUP']['DAYS']]) ?></span>
				</div>
			</div>
			...
			<div id="DPD_modController" class="<?= count($arResult['TERMINAL_TYPES']) > 1 ? '' : 'dpd-hidden' ?>">
				...
				<div class="DPD_mC_block <?= in_array(
                    GetMessage('IPOLH_DPD_PICKUP_TERMINAL_TYPE_POSTOMAT_KEY'),
			...
			<div>
				<div id="DPD_wrapper">
                    ...
                    if ($arResult['TERMINALS']) { ?>
                        <?php
                        foreach ($arResult['TERMINALS'] as $arTerminal) { ?>
							<p class="DPD_terminalSelect" data-terminal-code="<?= $arTerminal['CODE'] ?>"
							   data-terminal-address="<?= htmlspecialchars($arTerminal['ADDRESS_FULL']) ?>"><?= $arTerminal['NAME'] ?></p>
                            <?php
                        } ?>
                        <?php
                    } ?>
