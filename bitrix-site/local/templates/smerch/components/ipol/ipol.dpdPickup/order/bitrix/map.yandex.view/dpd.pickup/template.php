...
if ($arParams['BX_EDITOR_RENDER_MODE'] == 'Y') { ?>
	...
	<script type="text/javascript">
		function BX_SetPlacemarks_<?echo $arParams['MAP_ID']?>(map) {
            ...
            for($i = 0; $i < $cnt; $i++):
            ?>
			arObjects.PLACEMARKS[arObjects.PLACEMARKS.length] = BX_YMapAddPlacemarkDPD(map, <?echo CUtil::PhpToJsObject(
                $arResult['POSITION']['PLACEMARKS'][$i]
            )?>);
            <?
            endfor;
            endif;
	...
	<div class="bx-yandex-view-layout">
		<div class="bx-yandex-view-map">
            <?php
            app()->IncludeComponent('bitrix:map.yandex.system', '.default', $arTransParams, $component->GetParent(), ['HIDE_ICONS' => 'Y']) ?>
		</div>
	</div>
    <?php
}
