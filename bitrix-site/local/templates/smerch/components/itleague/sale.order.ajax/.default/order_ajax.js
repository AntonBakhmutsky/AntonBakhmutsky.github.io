...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		showError: function(node, msg, border)
		{
			...
			msg = msg.replace('ipol.dpd: Не заполнено обязательное поле "Код терминала"', 'Не выбран пункт выдачи заказа')
		...
		editRegistrationForm: function(authContent)
		{
			...
			else
			{
				...
				{
					authFormNodes.push(BX.create('DIV', {
						props: {className: 'bx-authform-social registration'},
				...
				authFormFields.push(this.createAuthFormInputContainer(
						...
						children: [
								...
								BX.create('INPUT', {
									props: {
										name: 'NEW_LOGIN',
										type: 'hidden',
										value: this.result.AUTH.NEW_LOGIN || 'generated_login'
									},
									style: {display: 'none'}
								}),
				...
				authFormNodes.push(
					BX.create('DIV', {
						props: {className: 'bx-authform-formgroup-container'},
						children: BX('bx-soa-user-consent').innerHTML
					}))
			...
			if (this.result.AUTH.new_user_registration === 'Y')
			{
				nodes.push(BX.create('DIV', {
					...
					children: [
						...
						BX.create('A', {
							props: {
								href: document.location.href + '?skip_registration=yes',
				...
				BX.create('DIV', {
					...
					children: [
						BX.create('h2', {
							...
							html: '<span class="bx-soa-section-title-count"></span>' + (BX.hasClass(insertContainer, 'reg') ? this.params.MESS_REG_BLOCK_NAME : this.params.MESS_AUTH_BLOCK_NAME)
						})
					]
				})
			);
			...
			if (this.options.showPropsInBasket && props.length)
			{
				for (var i in props)
				{
					if (props.hasOwnProperty(i))
					{
						...
						propsNodes.push(
							BX.create('DIV', {
								props: {className: `bx-soa-item-td-size bx-soa-item-td-size_${
									(Number(value.toLowerCase().replace('/', '').split(' ')[0])) ? 'cert' : value.toLowerCase().replace('/', '')
								}`},
								style: {textAlign: 'left'},
								text: (Number(value.toLowerCase().replace('/', '').split(' ')[0])) ? value.toLowerCase().replace('/', '') : null
							})
						);
					}
				}
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				toRight = BX.util.in_array(column.id, ["QUANTITY", "PRICE_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED", "SUM"]),
				textNode = BX.create('DIV', {props: {className: 'bx-soa-item-td-text price'}}),
				logotype, img;
			...
			var couponsList = this.getCouponsList(true),
				...
				couponsBlock = BX.create('DIV', {
					...
					children: [
						...
						BX.create('DIV', {
							props: {className: 'bx-soa-coupon-input-container'},
							children: [
		...
		getProfilesControl: function(node)
		{
			var profilesLength = BX.util.object_keys(this.result.USER_PROFILES).length,
				i, label, options = [],
				profileChangeInput, input, inputWrap;
			...
			if (profilesLength)
			{
				...
				)
				{
					...
					label = BX.create('LABEL', {
						props: {className: 'bx-soa-custom-label select-label'},
					...
					inputWrap = BX.create('DIV', {
						props: {className: 'select-wrap'},
						children: [input],
					});
					...
					node.appendChild(
						BX.create('DIV', {
							props: {className: 'form-group bx-soa-location-input-container'},
							children: [label, profileChangeInput, inputWrap]
						}),
					);
				}
				else
		...
		editFadePaySystemContent: function(node)
		{
			...
			if (selectedPaySystem && selectedPaySystem.NAME)
			{
				...
				addedHtml += '<div class="bx-soa-pp-company-selected ' + selectedPaySystem.CODE + '">';
				// addedHtml += '<img src="' + imgSrc + '" style="height:18px;" alt="">';
		...
		editFadeDeliveryContent: function(node)
		{
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
				...
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'col-sm-9 bx-soa-pp-company-selected sort ' + 'sort_' + selectedDelivery.SORT},
						children: arNodes
					})
				);
				node.appendChild(
		...
		editPropsItems: function(propsNode)
		{
			...
			while (group = groupIterator()) {
				var propsGroupContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-group'}}),
					emptyGroup = true;
				...
				while (property = propsIterator()) {
					if (
						...
						continue;
					emptyGroup = false;
					this.getPropertyRowNode(property, propsGroupContainer, false);
				}

				if (!emptyGroup) {
					propsGroupContainer.prepend(BX.create('DIV', {
						props: {className: 'col-sm-12 bx-soa-group-title'},
						html: group.getName()
					}))
				}
				propsItemsContainer.appendChild(propsGroupContainer);
			}
		...
		isValidForm: function()
		{
			...
			if (this.params.USER_CONSENT === 'Y') {
				const agreementCheckbox = BX.findChild(this.orderSaveBlockNode, {
					tag: 'input',
					attribute: {name: 'ORDER_USER_AGREEMENT', type: 'checkbox'}
				}, true)

				BX.removeClass(this.orderBlockNode, 'bx-step-error');
				const errorNode = this.orderBlockNode.querySelector('div.alert.alert-danger');
				errorNode.style.display = 'none';
				BX.cleanNode(errorNode);

				if (!agreementCheckbox.checked) {
					this.showError(this.orderBlockNode, 'Необходимо согласие с политикой конфиденциальности и пользовательским соглашением')
					BX.addClass(this.orderBlockNode, 'bx-step-error')
					this.animateScrollTo(this.orderBlockNode, 800, 50);
					return false
				}
			}
