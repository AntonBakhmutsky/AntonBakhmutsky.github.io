...
if (! empty($arResult['ERRORS']['FATAL'])) {
	...
	<div id="sls-<?= $arResult['RANDOM_TAG'] ?>" class="bx-sls <?= $arResult['MODE_CLASSES'] <> '' ? $arResult['MODE_CLASSES'] : '' ?>">
		...
		<div class="<?= $dropDownBlock ?> bx-ui-sls-input-block form-control">
			
			<span class="dropdown-icon"></span>
			<input type="text" autocomplete="off" name="<?= $arParams['INPUT_NAME'] ?>" value="<?= $arResult['VALUE'] ?>" class="dropdown-field"
	...
	<script>
		...
		window.BX.locationsDeferred['<?=$arParams['JS_CONTROL_DEFERRED_INIT']?>'] = function () {
            ...
            <?endif?>
				...
				new BX.Sale.component.location.selector.search(<?=CUtil::PhpToJSObject(
                    [
