if (typeof (BX.SalePersonalProfileListComponent) === 'undefined') {

	BX.SalePersonalProfileListComponent = function (id) {
		this._id = id

		const buttons = BX.findChildren(BX(this._id), {tag: 'button'}, true)
	...
	BX.SalePersonalProfileListComponent.prototype =
		{
			...
			showConfirmPopup: function (urlToDelete) {
				this._popup.setTitle('Вы точно хотите удалить адрес?\n' +
				...
				this._popup.setButtons([
					...
					new BX.PopupButton({
						text: 'Да, удалить',
						className: "popup-window-button_fill",
						events: {
							click: function () {
			...
			delete: function (event) {
				...
				this.showConfirmPopup(event.target.closest('button').dataset.urlToDelete)
			}
		}
	BX.SalePersonalProfileListComponent.create = function (id) {
		const _self = new BX.SalePersonalProfileListComponent(id)
		_self.initialize(id)
		return _self
	};
}
