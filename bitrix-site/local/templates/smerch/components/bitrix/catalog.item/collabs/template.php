...
if (isset($arResult['ITEM'])) {
    ...
    $picture = CFile::ResizeImageGet($item['PREVIEW_PICTURE']['ID'], ['width' => 432, 'height' => 432], BX_RESIZE_IMAGE_PROPORTIONAL, true);
    ?>
	
	<a class="col-tile__item" id="<?= $areaId ?>" href="<?= $item['DETAIL_PAGE_URL'] ?>" data-entity="item">
		<div class="col-tile__item-image" data-entity="image-wrapper">
			<img src="<?= $picture['src'] ?>" alt="<?= $imgAlt ?>"
			     width="<?= $picture['width'] ?>" height="<?= $picture['height'] ?>"
                <?= $imgTitle ? 'title="' . $imgTitle . '"' : '' ?>>
		</div>
		<div class="col-tile__item-name"><?= $item['NAME'] ?></div>
        <?php
        if (! empty($minPrice)) {
            ?>
        ...
        if ($canBuy) { ?>
			...
			<div class="col-tile__item-absent">НЕТ В НАЛИЧИИ</div>
            <?php
        } ?>
