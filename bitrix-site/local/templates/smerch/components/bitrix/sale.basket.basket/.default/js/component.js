;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		initProductPopup: function () {
			this.productPopup = BX.PopupWindowManager.create(this.ids.basketRoot + '_popup', null, {
				autoHide: false,
				closeByEsc: true,
				closeIcon: true,
				className: 'popup-window_basket',
				contentColor: 'white',
				overlay: true,
				contentPadding: null,
				padding: null,
			...
			if(this.result['BASKET_POPUP_PRODUCT_ID'] !== '') {
				this.showProductPopup(this.result['BASKET_POPUP_PRODUCT_ID']);
			}
		},

		showProductPopup: function (productId) {
			this.productPopup.setTitle('Рекомендуем добавить носочки')

			const popup = this.productPopup;
			BX.ajax({
				url: '/personal/cart/basket_popup_product.php',
				data: {
		...
		deleteBasketItem: function(itemId, restore, final)
		{
			...
			if (!this.shownItems || !this.shownItems.length) {
