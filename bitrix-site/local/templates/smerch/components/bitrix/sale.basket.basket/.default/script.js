...
BX.ready(function() {
	...
	if (document.querySelector('.basket-item-property.RATING')) {
		document.querySelectorAll('.basket-item-property.RATING .basket-item-property-value li').forEach(el => {
			el.innerHTML = `<span class="basket-item-scu-item-inner">${el.textContent.trim().split(' ')[0]}</span>`
		})
	}
});
